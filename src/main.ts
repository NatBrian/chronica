...
import { mountSeedBrowser, mountLayerViewer } from './ui/devtools';
...
function startWorld(seed: number): void {
  ...
  // mouse: drag pan + wheel zoom-to-cursor
  ...
  function drawDynamic(): void {
    ...
    // pawns as dots (visible from region zoom up)
    if (cam.pxPerTile >= 4) {
      const p = latest.pawns;
      const sz = cam.pxPerTile >= 16 ? 3 : 1;
